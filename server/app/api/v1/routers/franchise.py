...
class FeeConfigCreate(BaseModel):
    fee_type: str = Field(..., pattern="^(royalty|marketing|technology|initial|renewal|other)$")
    calculation_type: str = Field("percentage", pattern="^(percentage|fixed|tiered|per_transaction)$")
    rate: float = Field(0.0, ge=0, le=1)  # For percentage (0.05 = 5%)
    fixed_amount: float = Field(0.0, ge=0)
    min_amount: Optional[float] = None
    max_amount: Optional[float] = None
    tier_brackets: Optional[List[dict]] = None  # [{"min": 0, "max": 10000, "rate": 0.06}]
...
def calculate_fee(
    ...
    if calculation_type == "percentage":
        ...
        try:
            brackets = json.loads(tier_brackets) if isinstance(tier_brackets, str) else tier_brackets
...
def list_fee_configs(
    ...
    query = db.query(FranchiseFeeConfig).filter(FranchiseFeeConfig.tenant_id == tenant.id)
    ...
    for config in configs:
        config_dict = {
            ...
            "tier_brackets": json.loads(config.tier_brackets) if config.tier_brackets else None,
...
@router.post("/fees/config", response_model=FeeConfigResponse, status_code=status.HTTP_201_CREATED)
def create_fee_config(
    ...
    existing = db.query(FranchiseFeeConfig).filter(
    ...
    config = FranchiseFeeConfig(
        ...
        tier_brackets=json.dumps(config_data.tier_brackets) if config_data.tier_brackets else None,
    ...
    logger.info(f"Franchise fee config created: {config.fee_type} for tenant {tenant.id}")
...
def update_fee_config(
    ...
    config = db.query(FranchiseFeeConfig).filter(
    ...
    if "tier_brackets" in update_dict:
        update_dict["tier_brackets"] = json.dumps(update_dict["tier_brackets"]) if update_dict["tier_brackets"] else None
...
def calculate_fees(
    ...
    # Get fee configs
    configs = db.query(FranchiseFeeConfig).filter(
        FranchiseFeeConfig.tenant_id == tenant.id,
    ...
    if not configs:
        raise HTTPException(status_code=404, detail="No active fee configurations found")
    ...
    for config in configs:
        ...
        results.append(FeeCalculationResult(
            fee_type=config.fee_type,
            gross_sales=gross_sales,
            transaction_count=transaction_count,
...
def generate_fee_records(
    ...
    for config in configs:
        ...
        existing = db.query(FranchiseFeeRecord).filter(
...
def list_fee_records(
    ...
    query = db.query(FranchiseFeeRecord).filter(FranchiseFeeRecord.tenant_id == tenant.id)
    ...
    records = query.order_by(FranchiseFeeRecord.period_start.desc()).offset(skip).limit(limit).all()
...
def mark_fee_paid(
    ...
    record = db.query(FranchiseFeeRecord).filter(
...
def get_franchise_summary(
    ...
    records = db.query(FranchiseFeeRecord).filter(
        FranchiseFeeRecord.tenant_id == tenant.id,
    ...
    # Store count
    store_count = db.query(func.count(Store.id)).filter(
