...
def list_connections(
    ...
    connections = db.query(IntegrationConfig).filter(
    ...
    for conn in connections:
        result.append(ConnectionResponse(
            ...
            last_sync=str(conn.last_sync_at) if hasattr(conn, 'last_sync_at') and conn.last_sync_at else None,
...
async def initiate_connection(
    ...
    try:
        ...
        raise HTTPException(status_code=400, detail=f"Unsupported provider: {connect_data.provider}")
...
async def disconnect(
    ...
    connection = db.query(IntegrationConfig).filter(
...
async def trigger_sync(
    connection_id: int,
    sync_type: str = Query("all", pattern="^(all|products|orders|inventory|customers)$"),
    db: Session = Depends(get_db),
    current_user=Depends(get_current_user),
):
...
async def get_connection_status(
    ...
    return {
        ...
        "sync_direction": connection.sync_direction.value if hasattr(connection.sync_direction, 'value') else str(connection.sync_direction),
        "last_sync_at": str(connection.last_sync_at) if hasattr(connection, 'last_sync_at') and connection.last_sync_at else None,
