...
def get_user_stores(db: Session, user_id: int) -> List[Store]:
    ...
    if not tenant_user.store_ids:
        return db.query(Store).filter(
            Store.tenant_id == tenant_user.tenant_id,
...
def generate_transfer_number(db: Session, tenant_id: int) -> str:
    ...
    count = db.query(func.count(StoreTransfer.id)).filter(
...
def create_store(
    ...
    if subscription:
        ...
        if plan:
            current_store_count = db.query(func.count(Store.id)).filter(
...
def delete_store(
    ...
    active_count = db.query(func.count(Store.id)).filter(
        Store.tenant_id == tenant.id,
        Store.is_active == True
    ).scalar() or 0
    
    if active_count <= 1:
        raise HTTPException(status_code=400, detail="Cannot delete the last active store")
...
def list_transfers(
    ...
    if store_id:
        if direction == "incoming":
            ...
            query = query.filter(
                (StoreTransfer.from_store_id == store_id) | (StoreTransfer.to_store_id == store_id)
            )
    
    if status:
        query = query.filter(StoreTransfer.status == status)
    
    transfers = query.order_by(StoreTransfer.requested_at.desc()).offset(skip).limit(limit).all()
    return transfers


@router.post("/transfers", response_model=TransferWithItems, status_code=status.HTTP_201_CREATED)
def create_transfer(
    ...
    from app.db.multistore_models import StoreTransfer, StoreTransferItem, TransferHistory
    ...
    # Validate stores
    from_store = db.query(Store).filter(
        Store.id == transfer_data.from_store_id,
        Store.tenant_id == tenant.id
    ).first()
    
    to_store = db.query(Store).filter(
        Store.id == transfer_data.to_store_id,
        Store.tenant_id == tenant.id
    ).first()
    ...
    logger.info(f"Transfer {transfer.transfer_number} created from store {from_store.code} to {to_store.code}")
    
    # Build response
    response = TransferWithItems.model_validate(transfer)
    response.items = [TransferItemResponse.model_validate(item) for item in transfer.items]
...
def get_transfer(
    ...
    transfer = db.query(StoreTransfer).filter(
        StoreTransfer.id == transfer_id,
        StoreTransfer.tenant_id == tenant.id
    ).first()
    ...
    response = TransferWithItems.model_validate(transfer)
    response.items = [TransferItemResponse.model_validate(item) for item in transfer.items]
    response.from_store = StoreResponse.model_validate(from_store) if from_store else None
...
def ship_transfer(
    ...
    if transfer.status != "approved":
        raise HTTPException(status_code=400, detail=f"Cannot ship transfer with status '{transfer.status}'")
...
def cancel_transfer(
    ...
    return {"message": "Transfer cancelled", "transfer_number": transfer.transfer_number}
