...
class StoreTransfer(Base):
    ...
    status: Mapped[str] = mapped_column(
        String(50), default=TransferStatus.PENDING.value
    )
    priority: Mapped[str] = mapped_column(
        String(20), default=TransferPriority.NORMAL.value
    )
    ...
    expected_delivery: Mapped[Optional[datetime]] = mapped_column(
    ...
    # Relationships
    items: Mapped[List["StoreTransferItem"]] = relationship(
...
class StoreTransferItem(Base):
    ...
    transfer_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("store_transfers.id"), nullable=False
    )
    ...
    quantity_sent: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )  # May differ from requested
    quantity_received: Mapped[Optional[int]] = mapped_column(
        Integer, nullable=True
    )  # May differ (damaged, etc.)
...
class TransferHistory(Base):
    ...
    )  # created, approved, shipped, received, etc.
    ...
    __table_args__ = (Index("ix_transfer_history_transfer", "transfer_id"),)
...
class FranchiseFeeConfig(Base):
    ...
    fee_type: Mapped[str] = mapped_column(
        String(50), nullable=False
    )  # royalty, marketing, etc.
    calculation_type: Mapped[str] = mapped_column(
        String(50), default=FeeCalculationType.PERCENTAGE.value
    )

    # Amounts
    rate: Mapped[float] = mapped_column(
        Numeric(5, 4), default=0.0
    )  # For percentage (e.g., 0.05 = 5%)
...
class FranchiseFeeRecord(Base):
    ...
    config_id: Mapped[int] = mapped_column(
        Integer, ForeignKey("franchise_fee_configs.id"), nullable=False
    )
...
class StoreAnalyticsSnapshot(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "store_id", "snapshot_date", "period_type", name="uq_store_snapshot"
        ),
    )
