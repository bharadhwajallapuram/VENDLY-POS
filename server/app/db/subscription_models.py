...
class Plan(Base):
    ...
    tier: Mapped[str] = mapped_column(String(50), nullable=False, default=PlanTier.FREE.value)
    ...
    # Stripe Price IDs
    stripe_price_monthly_id: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    stripe_price_yearly_id: Mapped[Optional[str]] = mapped_column(String(100), nullable=True)
    ...
    is_public: Mapped[bool] = mapped_column(Boolean, default=True)  # Show on pricing page
...
class Tenant(Base):
    ...
    tenant_users: Mapped[List["TenantUser"]] = relationship(back_populates="tenant", cascade="all, delete-orphan")
...
class Store(Base):
    ...
    name: Mapped[str] = mapped_column(String(255), nullable=False)
    code: Mapped[str] = mapped_column(String(50), nullable=False)  # Short code like "STORE01"
    ...
    # Settings
    timezone: Mapped[Optional[str]] = mapped_column(String(50), nullable=True)  # Override tenant timezone
    is_active: Mapped[bool] = mapped_column(Boolean, default=True)
...
class TenantUser(Base):
    ...
    role: Mapped[str] = mapped_column(String(50), default="member")  # owner, admin, manager, member
    
    # Store access (null = all stores)
    store_ids: Mapped[Optional[str]] = mapped_column(Text, nullable=True)  # JSON array of store IDs
...
class Subscription(Base):
    ...
    # Status
    status: Mapped[str] = mapped_column(String(50), default=SubscriptionStatus.TRIALING.value)
    billing_interval: Mapped[str] = mapped_column(String(20), default=BillingInterval.MONTHLY.value)
...
class UsageEvent(Base):
    ...
    event_type: Mapped[str] = mapped_column(String(50), nullable=False)  # transaction, api_call, etc.
