...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column('provider', sa.Enum('quickbooks', 'xero', 'freshbooks', 'wave', 'sap', 'oracle_netsuite', 'microsoft_dynamics', 'shopify', 'woocommerce', 'magento', 'etsy', 'amazon', 'square', 'clover', name='integrationprovider'), nullable=False),
        ...
        sa.Column('sync_direction', sa.Enum('inbound', 'outbound', 'bidirectional', name='syncdirection'), default='bidirectional'),
    ...
    op.create_table(
        ...
        sa.Column('sync_type', sa.Enum('sales', 'inventory', 'customers', 'products', 'payments', 'orders', name='synctype'), nullable=False),
        sa.Column('status', sa.Enum('pending', 'in_progress', 'completed', 'failed', 'partial', name='syncstatus'), default='pending'),
    ...
    op.create_index(op.f('ix_integration_sync_logs_config_id'), 'integration_sync_logs', ['config_id'], unique=False)
    op.create_index(op.f('ix_integration_sync_logs_sync_type'), 'integration_sync_logs', ['sync_type'], unique=False)
    ...
    op.create_table(
        ...
        sa.Column('processing_status', sa.Enum('pending', 'in_progress', 'completed', 'failed', 'partial', name='syncstatus'), nullable=True),
    ...
    op.create_index(op.f('ix_integration_webhooks_processed'), 'integration_webhooks', ['processed'], unique=False)
