...
class TestSalesEndpoints:
    ...
    def test_create_sale(self, client: TestClient, auth_headers: dict, sample_product: dict):
    ...
    def test_create_sale_updates_inventory(self, client: TestClient, auth_headers: dict, sample_product: dict):
    ...
    def test_create_sale_without_auth(self, client: TestClient):
        ...
        sale_data = {
            "items": [{"product_id": 1, "quantity": 1, "unit_price": 10, "discount": 0}],
            "payment_method": "cash",
            "discount": 0
        }
    ...
    def test_create_sale_with_invalid_product(self, client: TestClient, auth_headers: dict):
    ...
    def test_get_sale_by_id(self, client: TestClient, auth_headers: dict, sample_product: dict):
    ...
    def test_list_sales(self, client: TestClient, auth_headers: dict, sample_product: dict):
    ...
    def test_sale_total_calculation(self, client: TestClient, auth_headers: dict, sample_product: dict):
    ...
    def test_create_sale_with_multiple_items(self, client: TestClient, auth_headers: dict):
        ...
        sale_data = {
            "items": [
                {"product_id": product1["id"], "quantity": 2, "unit_price": 10.00, "discount": 0},
                {"product_id": product2["id"], "quantity": 1, "unit_price": 20.00, "discount": 0}
            ],
            "payment_method": "cash",
            "discount": 0
        }
    ...
    def test_get_sale_receipt(self, client: TestClient, auth_headers: dict, sample_product: dict):
        ...
        # Get receipt
        response = client.get(
            f"/api/v1/sales/{sale_id}/receipt?format=text",
            headers=auth_headers
        )
        
        assert response.status_code == 200
        assert "Sale Test Product" in response.text or "RECEIPT" in response.text.upper()
